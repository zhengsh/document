...
* 如垃圾优先垃圾收集器一节中所述，G1 GC是区域化和粉黛垃圾收集器，这意味着Java对象堆被分成许多大小相等的区域，启动时，Java虚拟机（JVM）
设置区域大小，区域大小可以从1Mb 到 32MB不等，具体取决于堆大小。目标是不超过2018个地区。伊甸区，幸存者和老年代是这些分区的集合，并不是连续的。

* G1 GC具有尝试满足的暂停时间目标（软实时），在年轻的系列中，G1 GC调整其为年轻代（伊甸区和幸存区大小）。以满足软实时目标。

* 在混合集合期间，G1 GC 根据混合垃圾收集的目标数量，堆的每个区域活动对象的百分比，以及总体可接受的堆垃圾百分比来调整收集的旧区域数量。

* G1 GC 通过将活动对象从一组或多组区域（称为集合集（CSet）增量并行复制到一个或多个不同的新区域来减少堆碎片，以实现压缩，目标尽可能多地

* G1 GC 使用独立的记忆集（RSets）来跟踪对区域的引用，独立的RSet支持并行和独立的区域集合，因为只有区域的RSets必须被扫描以进入该区域，而不是整个堆
G1 GC 使用写后屏障来记录对堆的更改并更新RSets.

#### 垃圾收集阶段

* 除了疏散暂停，组成该停止的世界 STW， 年轻代，和混合垃圾回收的G1 GC还具有平行，同时，和多次刻印次数。G1 GC使用开始的快照 （SATB）算法
该算法在标记周期开始时逻辑上获取堆中活动对象集的快照。活动对象集还包括自标记周期开始以来分配的对象。G1 GC标记算法使用预写屏障来记录和标记作为
逻辑快照的一部分的对象。
   
#### 年轻代的垃圾收集

  * G1 GC 满足添加到伊甸去域集的区域的大多数分配请求。在年轻垃圾收集期间，G1 GC从之前的垃圾收集汇总收集伊甸区和幸存区域。来自伊甸区和
幸存区的活体被复制或者撤离到一组新的区域，特定对象的目标区域取决于对象的年龄。已老化的物体充分疏散到老年代，否者，该物体册立到幸存区，并将被
包括在下一个年轻代或混合垃圾收集的CSet中。

#### 混合垃圾收集

  *  成功完成并发标记周期后，G1 GC将从执行年轻垃圾收集切换到执行混合垃圾收集。在混合垃圾收集中，G1 GC 可选地将一些旧区域添加到将被收集的
伊甸去和幸存区中，添加的旧区域的确切数量由许多标志控制。 G1 GC 收集足够的旧区域后，G1 将恢复执行年轻垃圾收集，知道下一个标记周期完成。

#### 标记周期的阶段

  * 标记周期包括下面几个阶段
    * 初始标记阶段：G1 GC 在此阶段标记更。这个阶段在普通（STW）年轻垃圾收集上捎带。
    * 根区扫描阶段：G1 GC 烧苗初始标记阶段标记的幸存区，引用年老代，并标记引用的对象。此阶段与应用陈旭（而不是STW）同时运行，并且必须在下
一个STW年轻垃圾收集开始之前完成。
    * 并发标记节点：G1 GC在整个堆中查找可到达（实时）对象。此阶段与应用程序同时发生，并且可以被STW年轻垃圾收集中断。
...
#### 建议
  * 在评估和微调G1 GC 时，记住以下建议
    * 年轻代大小：避免使用 -Xmn 选项或任何其他相关选项明确设定年轻代大小-XX:NewRatio。修复年轻一代的大小会覆盖牧宝暂停时间目标。
    * 暂停时间目标：当您评估或调整任何垃圾收集时，始终存在延迟与吞吐量的权衡。G1 GC是一个增量垃圾收集器，具有统一的暂停。但在应用程序线程
上也有很多的开销，G1 GC的吞吐量目标是90%的应用程序时间和10%的垃圾回收时间。将其与Java HotSpot VM 并行收集器进行比较，并行收集器的吞吐量目标是
99%的应用程序时间和1%的垃圾收集时间。因此，当您评估G1 GC 的延迟时。您可以设置所需的（软）实时目标，G1 GC 将尝试满足它。作为副作用。吞吐量可能会受到
影响。
    * 驯服混合垃圾收集：调整混合垃圾收集是，请尝试一下选项。有关这些选项的信息。
