...
### CLH 锁
  CLH是一种基于单向链表的高性能、公平的自旋锁。申请加锁的线程通过前驱节点的变量进行自旋。在前置节点解锁后，当前节点会结束自选，并进行加锁。
在SMP架构下，CLH更具有优势。在NUMA架构下，若干当前节点与前驱节点在不同的CPU模块下，垮CPU模块会带来额外的开销，而MCS锁更适用于NUMA架构

加锁过程：
1. 获取当前下城的锁节点，如果为空，进行初始化。
...
### MCS 锁
  MSC与CLH最大的不同并不是链表是隐式还是显式，而是线程自选的规则不同：CLH是在前驱节点额locked域上自旋转等待，而MCS在自己的节点上locked域
上自选等待。正是如此，它解决了CLH在NUMA系统架构中获取locked域状态内存过远的问题。
MCS锁具体实现规则：
a. 队列初始化没有节点，taIl = null
b. 线程A想要获取锁，将自己置于队尾，由于它是一个节点，他的locaked域为false
c. 线程B和线程C相继加入队列，a -> next = b, b -> next = c, B 和C没有获取到锁，处于等待状态，所以locked域为true， 尾指针指向线程C对应的节点。
d. 线程A释放锁后，顺着它的next指正找到了线程B，并把B的locked 域设置为false ,这一动作会触发线程B获取锁。
Demo 如下：
```java
